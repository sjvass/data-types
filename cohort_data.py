...
def unique_houses(filename):
    ...
    houses = {(line.split("|")[2]) 
        for line in file if (line.split("|")[2]) != ""}

    # for line in file:
    #     line_data = line.split("|")
    #     if line_data[2] != "":
    #         houses.add(line_data[2])
        # if line_data[-1] != 'I' and line_data[-1] != 'G':
        #     student_list.append(line_data)
    file.close()
    return houses
...
def sort_by_cohort(filename):
    ...
    #for compressing into 1 line, use nested comprehension
    #list comprehension for all_students, then comprehensions for each cohort
    #within that comprehension

    for line in file:
        line = line.rstrip()
        line_data = line.split("|") 
        # if line_data[-1] != "I":
        if line_data[-1] == 'Winter 2016':
            winter_16.append((line_data[0]+" "+line_data[1]))
        elif line_data[-1] == 'Spring 2016':
            spring_16.append((line_data[0]+" "+line_data[1]))
        elif line_data[-1] == 'Summer 2016':
            summer_16.append((line_data[0]+" "+line_data[1]))
        elif line_data[-1] == 'Fall 2015':
            fall_15.append((line_data[0]+" "+line_data[1]))
        elif line_data[-1] == "G":
            ghosts.append((line_data[0]+" "+line_data[1]))
...
def all_students_tuple_list(filename):
    ...
    for line in file:
        ...
        line_data = line.split("|")
        if line_data[2] != "":
            # full_name = line_data[0] + " " + line_data[1]
            # line_data[:2] = full_name
            line_tuple = ((line_data[0] + " " + line_data[1]), line_data[2],
